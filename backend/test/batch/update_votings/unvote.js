...
module.exports = exports = function() {
    ...
    describe('unvote', () => {
        ...
        it('is_agree true', async function() {
            sinon.mock(VotingInfo)
                .expects('findOne')
                .atLeast(1)
                .atMost(10)
                .resolves(Object.assign({}, votingInfo, {is_agree : true, remove : async function() {}, save : async function() {}}));
            ...
            sinon.mock(UpdateVoting)
                ...
                .atLeast(1)
                .resolves([
                    {proposal_id : 1, account : 'useraaaaaabg', action : SEnum.VOTE_ACTION_UNVOTE, remove : async function() {}, save : async function() {}},
                ]);

            await updateVotings();

            const postProposal = await Proposal.findOne({proposal_id : 1});
            (proposal.total_votes - postProposal.total_votes).should.to.equal(votingInfo.staked);
            (proposal.agree_votes - postProposal.agree_votes).should.to.equal(votingInfo.staked);
            (proposal.disagree_votes).should.to.equal(postProposal.disagree_votes);
        });

        it('is_agree false', async function() {
            sinon.mock(VotingInfo)
                .expects('findOne')
                .atLeast(1)
                .atMost(10)
                .resolves(Object.assign({}, votingInfo, {is_agree : false, remove : async function() {}, save : async function() {}}));
            ...
            (proposal.total_votes - postProposal.total_votes).should.to.equal(votingInfo.staked);
            (proposal.disagree_votes - postProposal.disagree_votes).should.to.equal(votingInfo.staked);
            (proposal.agree_votes).should.to.equal(postProposal.agree_votes);
        });
    });
};
