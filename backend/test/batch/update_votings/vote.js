...
module.exports = exports = function() {
    ...
    describe('vote', () => {
        ...
        it('voting is agree and update is agree', async function() {
            sinon.mock(VotingInfo)
                .expects('findOne')
                .atLeast(1)
                .atMost(10)
                .resolves(Object.assign({}, votingInfo, {is_agree : true, save : async function() {}}));
            ...
            sinon.mock(UpdateVoting)
                ...
                .atLeast(1)
                .resolves([
                    {proposal_id : 1, account : 'useraaaaaabg', action : SEnum.VOTE_ACTION_VOTE, is_agree : true, remove : async function() {}, save : async function() {}},
                    // {proposal_id : 1, account : 'useraaaaaabg', action : SEnum.VOTE_ACTION_VOTE, is_agree : false, remove : async function() {}, save : async function() {}},
                ]);

            await updateVotings();

            const postProposal = await Proposal.findOne({proposal_id : 1});
            (postProposal.total_votes - proposal.total_votes).should.to.equal(voter.staked - votingInfo.staked);
            (postProposal.agree_votes - proposal.agree_votes).should.to.equal(voter.staked - votingInfo.staked);
            (postProposal.disagree_votes).should.to.equal(proposal.disagree_votes);
        });

        it('voting is agree and update is disagree', async function() {
            sinon.mock(VotingInfo)
                .expects('findOne')
                .atLeast(1)
                .atMost(10)
            ...
            const postProposal = await Proposal.findOne({proposal_id : 1});
            (postProposal.total_votes - proposal.total_votes).should.to.equal(voter.staked - votingInfo.staked);
            (postProposal.agree_votes - proposal.agree_votes).should.to.equal(-votingInfo.staked);
            (postProposal.disagree_votes - proposal.disagree_votes).should.to.equal(voter.staked);
        });

        it('voting is disagree and update is agree', async function() {
            sinon.mock(VotingInfo)
                .expects('findOne')
                .atLeast(1)
                .atMost(10)
                .resolves(Object.assign({}, votingInfo, {is_agree : false, save : async function() {}}));
            ...
            const postProposal = await Proposal.findOne({proposal_id : 1});
            (postProposal.total_votes - proposal.total_votes).should.to.equal(voter.staked - votingInfo.staked);
            (postProposal.disagree_votes - proposal.disagree_votes).should.to.equal(-votingInfo.staked);
            (postProposal.agree_votes - proposal.agree_votes).should.to.equal(voter.staked);
        });


        it('voting is disagree and update is disagree', async function() {
            sinon.mock(VotingInfo)
                .expects('findOne')
                .atLeast(1)
                .atMost(10)
            ...
            const postProposal = await Proposal.findOne({proposal_id : 1});
            (postProposal.total_votes - proposal.total_votes).should.to.equal(voter.staked - votingInfo.staked);
            (postProposal.disagree_votes - proposal.disagree_votes).should.to.equal(voter.staked - votingInfo.staked);
            (postProposal.agree_votes).should.to.equal(proposal.agree_votes);
        });
    });
};
